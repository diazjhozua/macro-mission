...
import { useFeed } from "@/lib/hooks/useSocial";

// Each page is fetched independently and accumulated in local state.
// Simple "load more" pattern — no infinite scroll library needed.
export default function FeedPage() {
  const [pages, setPages] = useState([1]);
  const queries = pages.map((page) => useFeed(page));
